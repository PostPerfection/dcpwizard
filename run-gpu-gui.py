...
def preferences_path():
    ...
    if sys.platform == "darwin":
        return Path.home() / "Library" / "Application Support" / "dcpwizard" / "preferences.json"
    if sys.platform == "win32" and (application_data := os.environ.get("APPDATA")):
        return Path(application_data) / "dcpwizard" / "preferences.json"
    return Path.home() / ".config" / "dcpwizard" / "preferences.json"
