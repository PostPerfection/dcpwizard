...
/// Build a JSON payload for a DCP validation result.
pub fn build_validation_payload(dcp_dir: &Path, valid: bool, errors: u32, warnings: u32) -> String {
    format!(
        r#"{{"dcp_dir":"{}","valid":{},"errors":{},"warnings":{}}}"#,
        escape_json_string(&dcp_dir.display().to_string()),
        valid,
        errors,
        warnings
    )
...
pub fn test_webhook(config: &WebhookConfig) -> WebhookResult {
    let event = WebhookEvent {
        ...
        job_id: String::new(),
        payload_json: r#"{"message":"DCP Wizard webhook test"}"#.to_string(),
...
mod tests {
    ...
    fn test_build_job_completed_payload() {
        let payload = build_job_completed_payload("job-1", Path::new("/output/dcp"), 125.5);
        assert!(payload.contains(r#""job_id":"job-1""#));
        assert!(payload.contains(r#""status":"completed""#));
        assert!(payload.contains(r#""output_dir":"/output/dcp""#));
    ...
    fn test_build_validation_payload() {
        let payload = build_validation_payload(Path::new("/data/dcp"), true, 0, 2);
        assert!(payload.contains(r#""dcp_dir":"/data/dcp""#));
    ...
    fn test_build_validation_payload_invalid() {
        let payload = build_validation_payload(Path::new("/data/dcp"), false, 3, 1);
