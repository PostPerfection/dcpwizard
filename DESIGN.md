...
- OV creation from video files or J2K dirs + WAV: demux, J2K encode (grok in-process FFI), real asdcplib MXF wrapping, CPL/PKL/ASSETMAP/VOLINDEX generation, SHA-1 hashes.
...
- MCA channel labeling: every PCM sound wrap carries ST 429-12 labels derived from the channel count (2.0/5.1/7.1); `create --hi-channel/--vi-channel` add the standalone HI/VI-N accessibility labels. dcpdoctor's MCA note is CPL-substring-based so it does not clear for MXF-only labels; verify with asdcplib `mca_labels`.
...
Subtitle packaging into a DCP timed-text track is now real: `create --subtitle <srt>` converts the SRT to ST 428-7 DCST XML, wraps it as a timed-text MXF and registers it in the reel/CPL/PKL/ASSETMAP.
